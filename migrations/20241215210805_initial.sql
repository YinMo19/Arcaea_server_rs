...
CREATE TABLE user (
  ...
  name TEXT UNIQUE,
  password TEXT,
  join_date INTEGER, -- Changed from char(20) to integer for timestamp
  user_code TEXT, -- Changed from char(10) to text
  rating_ptt INTEGER,
  character_id INTEGER,
  is_skill_sealed INTEGER,
  is_char_uncapped INTEGER,
  is_char_uncapped_override INTEGER,
  is_hide_rating INTEGER,
  song_id TEXT,
  difficulty INTEGER,
  score INTEGER,
  ...
  clear_type INTEGER,
  rating REAL,
  favorite_character INTEGER,
  max_stamina_notification_enabled INTEGER,
  current_map TEXT,
  ticket INTEGER,
  prog_boost INTEGER,
  email TEXT,
  world_rank_score INTEGER,
  ban_flag TEXT,
  next_fragstam_ts INTEGER,
  max_stamina_ts INTEGER,
  stamina INTEGER,
  world_mode_locked_end_ts INTEGER,
  beyond_boost_gauge REAL DEFAULT 0,
  kanae_stored_prog REAL DEFAULT 0,
  mp_notification_enabled INTEGER DEFAULT 1,
  highest_rating_ptt INTEGER DEFAULT 0,
  insight_state INTEGER DEFAULT 4
);
...
create table if not exists purchase (
  purchase_name text primary key,
  price int,
  orig_price int,
  discount_from int,
  discount_to int,
...
create table if not exists redeem (code text primary key, type int);
...
create table if not exists role (role_id text primary key, caption text);
...
create table if not exists power(power_id text primary key, caption text);
